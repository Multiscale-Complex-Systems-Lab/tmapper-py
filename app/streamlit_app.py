...
def generate_code(source_label, selected_vars, zscore_on, start_row, end_row, downsample,
                   lag, order, k, d, texclude, maxdistprct, maxdist, reciprocal,
    ...
    lines += [
        ...
        f"max_neighbor_dist_prct={maxdistprct!r}, max_neighbor_dist={maxdist!r}, reciprocal={reciprocal!r})",
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("Load sample data", use_container_width=True):
            ...
            sample_df = read_csv_smart(SAMPLE_DATA_PATH).iloc[53883:].reset_index(drop=True)
        ...
        if uploaded is not None and st.session_state.get("data_source") != f"uploaded: {uploaded.name}":
            set_data(read_csv_smart(uploaded), f"uploaded: {uploaded.name}")
    ...
    if "built" in st.session_state:
        ...
        render_network(built, df, color_var, time_var, nodesizemode, labelmethod, show_recurrence)

        with st.expander("Show equivalent code"):
            st.code(
                generate_code(
                    st.session_state["data_source"], selected_vars, zscore_on, start_row, end_row,
