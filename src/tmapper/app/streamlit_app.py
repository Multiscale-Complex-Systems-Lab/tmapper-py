...
# Above this many points, build the graph from coordinates a block of rows
# at a time (tknndigraph's low_memory path) instead of materialising the
# (N, N) distance matrix. Both paths produce the identical graph; the
# blocked one trades a little speed for memory that stops growing with N^2,
# so small windows stay on the dense path where it is faster.
LOW_MEMORY_ROWS = 8000
...
def build_network(
    ...
    # -- past LOW_MEMORY_ROWS the dense matrix is the binding constraint, not
    # the graph: cdist would allocate len(X)^2 float64s. tknndigraph's
    # low_memory path takes the coordinates instead and scans them a block of
    # rows at a time, so peak memory tracks block_size * N rather than N^2.
    # The graph is identical either way (the two paths were verified equal
    # across 540 parameter configurations), so this is purely a resource
    # choice -- the dense path stays for small windows because it is faster.
    use_low_memory = len(X) > LOW_MEMORY_ROWS
    g, par = tknndigraph(
        X if use_low_memory else cdist(X, X, metric="euclidean"),
        k, tidx,
...
def generate_code(source_code, selected_vars, zscore_on, start_row, end_row, downsample,
    ...
    # -- which tknndigraph path to emit. `n_points` is what the app actually
    # built on; without it (callers that only have the widget values) fall
    # back to the row range, which ignores rows dropped for missing data and
    # so can land on the other side of the threshold. That is harmless rather
    # than wrong: both paths build the identical graph, so a borderline
    # mismatch changes the route the script takes, not where it arrives.
    if n_points is None:
        est = len(range(start_row, (end_row + 1) if end_row is not None else start_row))
        n_points = est // max(downsample, 1) if end_row is not None else LOW_MEMORY_ROWS
    use_low_memory = n_points > LOW_MEMORY_ROWS
    ...
    lines = [
        ...
        *([] if use_low_memory else ["from scipy.spatial.distance import cdist"]),
    ...
    if use_low_memory:
        lines += [
            ...
            "# so the full pairwise distance matrix is never allocated. The graph is",
            f"# the same one a precomputed matrix gives; at {n_points} points that",
            f"# matrix alone would have needed ~{8 * n_points ** 2 / 1e9:.2f} GB.",
